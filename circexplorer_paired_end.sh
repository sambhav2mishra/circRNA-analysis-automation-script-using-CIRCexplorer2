...
mm39_kg_gtf=/path_to_your_annotationref_and_annotations/gencode_mm39/gencode.vM28.annotation.gtf
...
STAR_align=$(ls /path_to_your_fqfiles/fastq_files/*.fastq.gz | cut -d '/' -f 8 | cut -d '_' -f 1 | uniq); ## Please read about cut command first
for DR in $STAR_align;
do
        FL1=$(ls /path_to_your_fqfiles/fastq_files/${DR}_1.fastq.gz) ##
        FL2=$(ls /path_to_your_fqfiles/fastq_files/${DR}_2.fastq.gz) ##
        STAR --runThreadN 30  --genomeDir ${mm39_star_index} --readFilesIn <(gunzip -c ${FL1} ${FL2}) --outSAMtype BAM Unsorted --sjdbGTFfile ${mm39_kg_gtf} --chimSegmentMin 10 --outFileNamePrefix ${DR}_
done;
